...
void test_regular_list_reverse() {
  ...
  while (current != NULL && i < n) {
    int j = n - i - 1;
    TEST_CHECK_(current->value == array[j],
      "list[%d] equal to array[%d] (%d == %d)", i, j, current->value, array[j]);
   ...
   */
  if (!TEST_CHECK(current == NULL && i == n)) {
 ...
 *
 *   * It tests to make sure the reversed list is not NULL.
 *   * It tests to make sure the values stored in the first node in the
 ...
 */
struct node* list_from_array(int* array, int n) {
  struct node* node, * first = NULL, * last = NULL;
  ...
  for (int i = 0; i < n; i++) {
    ...
    /*
     * Put the new node at the last of the list.
     */
    if (last) {
      last->next = node;
    }
